    ...
    async def load_extensions(self):
        ...
        for extension in extensions:
            try:
                ...
                logger.error(f"💥 Unexpected error loading {extension}: {e}", exc_info=True)
                self.failed_extensions.append(extension)
        
        logger.info(f"📊 Extension Summary: {len(self.loaded_extensions)} loaded, {len(self.failed_extensions)} failed")
    ...
    async def sync_commands(self, guild_id: Optional[int] = None):
        ...
        try:
            ...
            if guild_id:
                ...
                logger.info(f"🔄 Syncing commands to guild {guild_id} for instant testing...")
            ...
            try:
                ...
                if len(synced) != len(existing_commands):
                    logger.info(f"📊 Command count changed: {len(existing_commands)} → {len(synced)}")
        ...
        while not self.is_closed() and not self._shutdown_requested:
            try:
                ...
                if int(time.time()) % 300 == 0:  # Every 5 minutes
                    logger.info(f"💓 Heartbeat - Guilds: {len(self.guilds)}, "
